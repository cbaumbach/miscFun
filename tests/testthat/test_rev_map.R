...
test_that("happy path", {
    x <- rev_map(list(a = c("A", "C"), b = c("B", "C")))
    expect_equal(x$A, "a")
    expect_equal(x$B, "b")
...
test_that("if there are no NA values the reverse of the reverse is the map itself modulo the order of elements", {
    map <- list(a = c("A", "B"), b = c("B", "C"))
    expect_equal(rev_map(rev_map(map)), map)
})

test_that("reversing a map twice removes keys without values", {
    x <- rev_map(rev_map(list(a = "A", b = NULL, c = integer())))
    expect_equal(names(x), "a")
})
